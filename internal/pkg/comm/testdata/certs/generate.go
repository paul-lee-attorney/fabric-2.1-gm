...
//generate an SM2 private key (SM2P256V1 curve)
func genKeySM2(name string) (*sm2.PrivateKey, error) {
	...
	pem.Encode(keyFile, &pem.Block{Type: "SM2 PRIVATE KEY", Bytes: keyBytes})
	keyFile.Close()
	return priv, nil
}
...
func genCertificateSM2(name string, template, parent *x509.Certificate, pub *sm2.PublicKey,
...
func genServerCertificateSM2(name string, signKey *sm2.PrivateKey, signCert *x509.Certificate) error {
	fmt.Println(name)
...
//generate an EC certificate appropriate for use by a TLS server
func genClientCertificateSM2(name string, signKey *sm2.PrivateKey, signCert *x509.Certificate) error {
	fmt.Println(name)
	key, err := genKeySM2(name)
	template, err := x509Template()
...
//generate a SM2 certificate signing(CA) key pair and output as
//PEM-encoded files
func genCertificateAuthoritySM2(name string) (*sm2.PrivateKey, *x509.Certificate, error) {
...
//generate a SM2 certificate appropriate for use by a TLS server
